...
By integrating with [Dataforsyningen](https://dawadocs.dataforsyningen.dk/dok/api/autocomplete), we will provide a
plug-and-play React Element implementing
...
- *acknowledging that this is an alternative to [dawa-autocomplete2](https://github.com/SDFIdk/dawa-autocomplete2),*
...
- *minimal dependencies other than React, React DOM, and optionally Bootstrap.*

### Terminology

A valid address is defined as having **necessarily**
a unique identifier with Dataforsyningen,
a type, being either "entrance" or "address",
a street name, house number, zipcode, and zipcode name; and as having **optionally**
a floor,
a door,
a street code,
...
a status,
a dar status,
a unique identifier with Dataforsyningen for its entrance address,
a latitude and a longitude,
and a href.

These are mapped from Dataforsyningen as according to `mapResDtoToAddress()`.

In [Dataforsyningen-terms](https://dawadocs.dataforsyningen.dk/dok/api/adgangsadresse), the difference between an "
entrance" and an "address" address is:
> An "address" encompasses potentially a floor and/or door designation. An "entrance" address does not.

That is, the "entrance" address of an "address" address does not designate its floor and/or door. And multiple "address"
addresses may reference the same "entrance" address through their unique identifier for "entrance" address. Currently,
with [dawa-autocomplete2](https://github.com/SDFIdk/dawa-autocomplete2), the consuming developer must determine that
**either** "entrance" **or** "address" constitutes an autocompletion. With **dawa-autocomplete-react**, this
responsibility is delegated to the user based on the assumption that they may or may not find an "entrance" sufficiently
precise for
their purpose. This means the `<input>` is technically valid for **both** "entrance" **and** "address", and the address
...
Insert as React Element:

```tsx
<DawaAutocompleteAddressInput addressObserver={(address) => {
}}/>
```

The component will style with [Bootstrap](https://getbootstrap.com) if it is installed
...
For instance, a danish translation may look like

```tsx
<DawaAutocompleteAddressInput configurationOverride={{
...
Lint and transpile to `out/src` and bundle the GitHub Pages demo at `out/test/index.html`:
...
- Compartmentalize by feature and domain, rather than by technology and architecture
...
- Appreciate that searching for addresses is modeled generically in
  `src/core/text-node-search` (see `create-resulting-caret-text-query-node`)
